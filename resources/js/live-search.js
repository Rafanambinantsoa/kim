    ...
    renderResultsHTML(posts) {
        if (posts.length) {
            ...
            return `<a href="/post/${post.id}" class="list-group-item list-group-item-action">
                        <img class="avatar-tiny" src="${post.cletrangere.avatar}"> <strong>${post.title}</strong>
                        <span class="text-muted small">by ${post.cletrangere.username} on ${postDate.getMonth() + 1}/${postDate.getDate()}/${postDate.getFullYear()}</span>
                    </a>`
            }).join("")}
            </div>`)
        } else {
